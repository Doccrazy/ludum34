...
public abstract class Level extends Box2dActor<GameWorld> {
    ...
    protected void spawnRandomStuff(float delta) {
        ...
        float fussballCount = 0;
        for (Actor actor : world.stage.getActors()) {
            if (actor instanceof FussballActor && !((FussballActor) actor).isDead()) {
                fussballCount++;
            }
        }
        if (MathUtils.randomBoolean(delta * fussballCount* dogPerSecPerFussball)) {
            world.addActor(new DogActor(world, getRandomBorderPoint()));
        }
    }
}
